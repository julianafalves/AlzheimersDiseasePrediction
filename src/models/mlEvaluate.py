...
def Evaluate(y_test,y_predict,prob_predict,dataName,isMulticlass,algorithm):
    ...
    if isMulticlass:
        metrics = {' algName': algorithm,
                'dataName' : dataName,
                #'auc' : [roc_auc_score(y_test,prob_predict,multi_class='ovo', average='weighted')],
    ...
    metrics_df = pd.DataFrame(metrics,columns=[' algName','dataName','auc','Accuracy','Precision','Recall','F1 Score'])
    ...
    try:
        ...
        result_df = pd.DataFrame(columns=[' algName','dataName','auc','Accuracy','Precision','Recall','F1 Score'])
